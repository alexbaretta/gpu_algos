...
// source path: src/benchmarks/hip/matrix_product/matrix_product_tensor.hip.cpp

#include <cxxopts.hpp>
#include "hip/benchmark.hip.hpp"
#include "hip/kernels/matrix_product/matrix_product_tensor.hip.hpp"

int main(int argc, char** argv) {
    cxxopts::Options options("matrix_product_tensor", "Matrix multiplication using tensor cores");
    ...
    try {
        ...
        if (spec.type_ == "half") {
            return Benchmark_2In_1Out<Matrix_product_tensor_kernel<_Float16>>(spec, options, options_parsed).run();
        } else if (spec.type_ == "single" || spec.type_ == "float") {
            return Benchmark_2In_1Out<Matrix_product_tensor_kernel<float>>(spec, options, options_parsed).run();
        } else if (spec.type_ == "double") {
            return Benchmark_2In_1Out<Matrix_product_tensor_kernel<double>>(spec, options, options_parsed).run();
        } else if (spec.type_ == "int8") {
            return Benchmark_2In_1Out<Matrix_product_tensor_kernel<std::int8_t>>(spec, options, options_parsed).run();
        } else if (spec.type_ == "int16") {
            return Benchmark_2In_1Out<Matrix_product_tensor_kernel<std::int16_t>>(spec, options, options_parsed).run();
        } else if (spec.type_ == "int32") {
            return Benchmark_2In_1Out<Matrix_product_tensor_kernel<std::int32_t>>(spec, options, options_parsed).run();
        } else if (spec.type_ == "int64") {
            return Benchmark_2In_1Out<Matrix_product_tensor_kernel<std::int64_t>>(spec, options, options_parsed).run();
