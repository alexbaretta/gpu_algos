...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (spec.type_ == "half") {
            ...
            return Benchmark_2In_1Out<Matrix_product_cublas_kernel<std::uint32_t>>(spec, options, options_parsed).run();
        } else if (spec.type_ == "uint64") {
            return Benchmark_2In_1Out<Matrix_product_cublas_kernel<std::uint64_t>>(spec, options, options_parsed).run();
        } else {
