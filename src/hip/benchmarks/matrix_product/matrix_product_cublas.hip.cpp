...
// source path: src/benchmarks/matrix_product/matrix_product_cublas.hip.cpp
...
#include "hip/kernels/matrix_product/matrix_product_cublas.hip.hpp"
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (spec.type_ == "half") {
            ...
            return Benchmark_Matrix_2In_1Out<Matrix_product_cublas_kernel<std::int16_t>>(spec, options, options_parsed).run();
        } else if (spec.type_ == "int32") {
            return Benchmark_Matrix_2In_1Out<Matrix_product_cublas_kernel<std::int32_t>>(spec, options, options_parsed).run();
        } else if (spec.type_ == "int64") {
            return Benchmark_Matrix_2In_1Out<Matrix_product_cublas_kernel<std::int64_t>>(spec, options, options_parsed).run();
            ...
            return Benchmark_Matrix_2In_1Out<Matrix_product_cublas_kernel<std::uint16_t>>(spec, options, options_parsed).run();
        } else if (spec.type_ == "uint32") {
            return Benchmark_Matrix_2In_1Out<Matrix_product_cublas_kernel<std::uint32_t>>(spec, options, options_parsed).run();
        } else if (spec.type_ == "uint64") {
            return Benchmark_Matrix_2In_1Out<Matrix_product_cublas_kernel<std::uint64_t>>(spec, options, options_parsed).run();
        } else {
