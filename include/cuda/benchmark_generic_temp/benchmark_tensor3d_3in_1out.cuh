...
// source path: include/cuda/benchmark_generic_temp/benchmark_tensor3d_3in_1out.cuh
...
class Benchmark_Tensor3D_3In_1Out {
    ...
    using Kernel = Tensor3D_Kernel_3In_1Out;
        ...
        kernel(spec, args...)
    {
        ...
        if (verbose && (
            ...
            || (spec.n_rows_D_ > 10000 || spec.n_cols_D_ * spec.n_sheets_D_ > 1000)
        )) {
    ...
    int run() {
        ...
        const size_t size_temp = size_t(spec.size_temp_);
        ...
        std::cout
            ...
            << "Input tensor3d C dimensions : " << spec.n_rows_C_ << "x" << spec.n_cols_C_ << "x" << spec.n_sheets_C_ << "\n"
            << "Output tensor3d dimensions  : " << spec.n_rows_D_ << "x" << spec.n_cols_D_ << "x" << spec.n_sheets_D_ << "\n"
            << "Temp size                   : " << size_temp << "\n"
        ...
        Tensor3D<NumberD> tensor3d_D(spec.n_cols_D_, spec.n_rows_D_, spec.n_sheets_D_, 0);
        // std::vector<NumberTemp> cpu_data_temp(size_temp, 0);
        ...
        if (is_random) {
            ...
            for (size_t i = 0; i < size_C; ++i) tensor3d_C.vector_[i] = NumberC(size_C - i);
        } else {
            std::cerr << "[ERROR] Invalid initialization method" << std::endl;
            exit(1);
        }
        ...
        cuda_check_error(cudaMemcpyAsync(gpu_data_B, tensor3d_B.data(), size_B_bytes, cudaMemcpyHostToDevice, stream), "cudaMemcpyAsync");
        cuda_check_error(cudaMemcpyAsync(gpu_data_C, tensor3d_C.data(), size_C_bytes, cudaMemcpyHostToDevice, stream), "cudaMemcpyAsync");
        ...
        cuda_check_error(cudaMemcpyAsync(tensor3d_D.data(), gpu_data_D, size_D_bytes, cudaMemcpyDeviceToHost, stream), "cudaMemcpyAsync");
        // if (size_temp_bytes > 0) {
        //     cuda_check_error(cudaMemcpyAsync(cpu_data_temp.data(), gpu_data_temp, size_temp_bytes, cudaMemcpyDeviceToHost, stream), "cudaMemcpyAsync");
        // }
        ...
        const auto tensor3d_D_cpu = kernel.run_host_kernel(tensor3d_A, tensor3d_B, tensor3d_C);
        const auto& tensor3d_result_gpu = tensor3d_D;
        const auto& tensor3d_result_cpu = tensor3d_D_cpu;
                ...
                for (int j = 0; j < tensor3d_E.cols(); ++j) {
                    for (int k = 0; k < tensor3d_E.sheets(); ++k) {
                        if (tensor3d_E(i, j, k) != 0.0) {
                            found_errors = true;
                            std::cout << "(" << i << ", " << j << ", " << k << "): "
                                    << "result gpu =" << static_cast<Printable_Number<NumberD>>(tensor3d_result_gpu(i, j, k)) << ", "
                                    << "result cpu =" << static_cast<Printable_Number<NumberD>>(tensor3d_result_cpu(i, j, k)) << ", "
