...
// source path: include/cuda/benchmark/benchmark_matrix_3in_1out.hpp
...
class Benchmark_Matrix_3In_1Out {
    ...
    Matrix_kernel_3In_1Out kernel;

    template <typename... Args>
    Benchmark_Matrix_3In_1Out(
        const Kernel_spec spec,
        ...
        kernel(spec, args...)
    {
        ...
        if (verbose && (spec.n_rows_A_ > 100000 || spec.n_cols_A_ > 100000 || spec.n_rows_B_ > 100000 || spec.n_cols_B_ > 100000 || spec.n_rows_C_ > 100000 || spec.n_cols_C_ > 100000)) {
    ...
    int run() {
        ...
        std::cout
            ...
            << "Input matrix B dimensions   : " << spec.n_rows_B_ << "x" << spec.n_cols_B_ << "\n"
            << "Input matrix C dimensions   : " << spec.n_rows_C_ << "x" << spec.n_cols_C_ << "\n"
            << "Output matrix dimensions    : " << spec.n_rows_D_ << "x" << spec.n_cols_D_ << "\n"
        ...
        std::cout << "MATRIX_KERNEL_3IN_1OUT:" << std::endl;
        ...
        const auto E = (D_gpu.template cast<double>() - D_cpu.template cast<double>()).template cast<Number>();
        ...
        double E_max = 0;
        long E_max_row = 0, E_max_col = 0;
        double E_max_pct = 0;
        long E_pct_max_row = 0, E_pct_max_col = 0;
        for (int i = 0; i < E.rows(); ++i) {
            for (int j = 0; j < E.cols(); ++j) {
                const double e = double(E(i, j));
                const double e_abs = std::abs(e);
                const double e_ref = double(D_cpu(i, j));
        ...
        std::cout << cpu_step_dt4.count() << " ms (" << cpu_total_dt4.count() << " ms total)" << std::endl;
        ...
        if (verbose) {
            ...
            std::cout << "C      :\n";
            std::cout << C.template cast<Printable_Number>().format(clean_matrix_format) << std::endl;
            std::cout << "D_gpu  :\n";
            std::cout << D_gpu.template cast<Printable_Number>().format(clean_matrix_format) << std::endl;
            std::cout << "D_cpu  :\n";
            std::cout << D_cpu.template cast<Printable_Number>().format(clean_matrix_format) << std::endl;
