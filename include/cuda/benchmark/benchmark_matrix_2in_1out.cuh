...
// source path: include/cuda/benchmark/benchmark_matrix_2in_1out.cuh
...
class Benchmark_Matrix_2In_1Out {
    ...
    using Kernel = Matrix_Kernel_2In_1Out;
    ...
    int run() {
        ...
        const Eigen::Map<Eigen::Matrix<NumberC, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> C_gpu{vec_C.data(), spec.n_rows_C_, spec.n_cols_C_};
        ...
        const auto C_cpu_raw = kernel.run_host_kernel(A, B);
        const auto C_cpu = C_cpu_raw.template cast<NumberC>();
                ...
                for (int j = 0; j < E.cols(); ++j) {
                    if (E(i, j) != NumberE(0)) {
                        ...
                        std::cout << "(" << i << ", " << j << "): "
                                  ...
                                  << "C_cpu=" << static_cast<Printable_Number<NumberC>>(C_cpu(i, j)) << ", "
                                  << "E=" << static_cast<Printable_Number<NumberC>>(E(i, j)) << "\n";
                    }
                }
            }
            if (!found_errors) {
        ...
        if (verbose) {
            ...
            std::cout << "C_gpu:\n";
            std::cout << C_gpu.template cast<Printable_Number<NumberC>>().format(eigen_format) << std::endl;
            std::cout << "C_cpu:\n";
            std::cout << C_cpu.template cast<Printable_Number<NumberC>>().format(eigen_format) << std::endl;
