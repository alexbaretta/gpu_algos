...
// source path: include/cuda/benchmark/benchmark_vector_1in_1out.cuh
...
class Benchmark_Vector_1In_1Out {
    ...
    using Kernel = Vector_Kernel_1In_1Out;
    ...
    Vector_Kernel_1In_1Out kernel;

    template <typename... Args>
    Benchmark_Vector_1In_1Out(
        const Kernel_spec spec,
    ...
    int run() {
        ...
        std::cout << "Vector_Kernel_1In_1Out:" << std::endl;
        ...
        const Eigen::Map<Eigen::Vector<NumberC, Eigen::Dynamic>> C_gpu{vec_C.data(), spec.n_C_};
        ...
        size_t E_max_idx, E_rel_max_idx;
        const double E_max = E.cwiseAbs().maxCoeff(&E_max_idx);
        const auto E_max_rel = E_rel.maxCoeff(&E_rel_max_idx);
            ...
            for (int i = 0; i < E.size(); ++i) {
                if (E(i) != NumberE(0)) {
                    ...
                    std::cout << "(" << i << "): "
                              ...
                              << "C_cpu=" << static_cast<Printable_Number<NumberC>>(C_cpu(i)) << ", "
        ...
        if (verbose) {
            ...
            std::cout << "C_gpu: ";
            std::cout << C_gpu.template cast<Printable_Number<NumberC>>().transpose().format(eigen_format) << std::endl;
            std::cout << "C_cpu: ";
            std::cout << C_cpu.template cast<Printable_Number<NumberC>>().transpose().format(eigen_format) << std::endl;
            if (spec.n_temp_ > 0) {
