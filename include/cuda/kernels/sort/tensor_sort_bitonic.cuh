...
struct tensor3d_sort_bitonic_spec {
    ...
    inline static tensor3d_sort_bitonic_spec make(
        ...
        // We do not declare these variables const as we might need to extend one to be a power of 2
        unsigned long m = options_parsed["M"].as<unsigned long>();
        unsigned long n = options_parsed["N"].as<unsigned long>();
        unsigned long k = options_parsed["K"].as<unsigned long>();
        ...
        const bool is_target_dim_a_power_of_two = ((target_dim & (target_dim - 1)) == 0);
        if (!is_target_dim_a_power_of_two) {
            std::cout << "[WARNING] " << sort_dim << " dimension=" << target_dim << " is not a power of 2, rounding down to ";
            target_dim = std::bit_floor(target_dim);
...
class tensor3d_sort_bitonic_kernel {
    ...
    void run_device_kernel(
        ...
        if ((n_elements & (n_elements - 1)) != 0) {
            ...
            throw std::runtime_error("Sort dimension size (" + std::to_string(n_elements) + ") is not a power of 2");
            return;
        }
        ...
        for (long step = 0; (1L << step) < n_elements; ++step) {
            ...
            for (long stage = 0; stage <= step; ++stage) {
                ...
                if (err != cudaSuccess) {
                    std::cerr << "[ERROR] CUDA kernel error: " << cudaGetErrorString(err) << std::endl;
                    return;
                }
            }
        }
