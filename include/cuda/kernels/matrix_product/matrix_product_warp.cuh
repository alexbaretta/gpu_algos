...
__global__ void matrix_product_warp(
    ...
    const long tid_warp = long(threadIdx.x);
    const long col = long(threadIdx.y) + long(blockIdx.x) * long(blockDim.y);
    const long row = long(threadIdx.z) + long(blockIdx.y) * long(blockDim.z);
    ...
    for (long i = tid_warp; i < k; i += warpSize) {
