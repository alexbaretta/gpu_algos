...
__global__ void matrix_product_tensor_wmma(
    ...
    const long x_wid_grid = long(blockIdx.x) * long(blockDim.y) + long(threadIdx.y);
    const long y_wid_grid = long(blockIdx.y) * long(blockDim.z) + long(threadIdx.z);
    const long wid_in_block = long(threadIdx.z) * long(blockDim.y) + long(threadIdx.y);
    ...
    for (long k_tile_i = 0; k_tile_i < k_tiles; ++k_tile_i) {
