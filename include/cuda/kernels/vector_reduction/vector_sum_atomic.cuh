...
__global__ void vector_reduction_with_atomics(
    const Number* A,
...
struct Vector_sum_atomic_spec {
    ...
    inline static Vector_sum_atomic_spec make(
    ...
    inline static Vector_sum_atomic_spec make(
        ...
        const auto [scalar_size, cuda_occupancy_kernel]  = (
            type == "half" ? std::make_tuple(sizeof(__half), (void*)vector_reduction_with_atomics<half>) :
            type == "single" || type == "float" ? std::make_tuple(sizeof(float), (void*)vector_reduction_with_atomics<float>) :
            type == "double" ? std::make_tuple(sizeof(double), (void*)vector_reduction_with_atomics<double>) :
            type == "int8" ? std::make_tuple(sizeof(int8_t), (void*)vector_reduction_with_atomics<int8_t>) :
            type == "int16" ? std::make_tuple(sizeof(int16_t), (void*)vector_reduction_with_atomics<int16_t>) :
            type == "int32" ? std::make_tuple(sizeof(int32_t), (void*)vector_reduction_with_atomics<int32_t>) :
            type == "int64" ? std::make_tuple(sizeof(int64_t), (void*)vector_reduction_with_atomics<int64_t>) :
            type == "uint8" ? std::make_tuple(sizeof(uint8_t), (void*)vector_reduction_with_atomics<uint8_t>) :
            type == "uint16" ? std::make_tuple(sizeof(uint16_t), (void*)vector_reduction_with_atomics<uint16_t>) :
            type == "uint32" ? std::make_tuple(sizeof(uint32_t), (void*)vector_reduction_with_atomics<uint32_t>) :
            type == "uint64" ? std::make_tuple(sizeof(uint64_t), (void*)vector_reduction_with_atomics<uint64_t>) :
        ...
        return make(type, scalar_size, cuda_occupancy_kernel, size, block_size);
    }
    ...
    inline static Vector_sum_atomic_spec make(
        ...
        const auto cuda_occupancy_kernel = vector_reduction_with_atomics<Number_>;
        return make(type_name(type_t<Number_>{}), scalar_size, (void*)cuda_occupancy_kernel, size, block_size);
    }
    ...
    inline static Vector_sum_atomic_spec make(
        ...
        return Vector_sum_atomic_spec(type, size, block_dim, grid_dim, shared_mem_size, n_temp);
    }

    protected:
    Vector_sum_atomic_spec(
        const auto& type,
        const long size,
...
static_assert(Check_vector_kernel_spec_1In_1Out<Vector_sum_atomic_spec>::check_passed, "Vector_sum_atomic_spec is not a valid kernel spec");


template <CUDA_scalar Number_>
class Vector_sum_atomic_kernel {
    public:
    using Number = Number_;
    using Kernel_spec = Vector_sum_atomic_spec;

    const Kernel_spec spec_;

    Vector_sum_atomic_kernel(
    ...
    void run_device_kernel(
        ...
        vector_reduction_with_atomics<Number><<<spec_.grid_dim_, spec_.block_dim_, spec_.shared_mem_size_, stream>>>(
            gpu_data_A,
            spec_.n_A_,
            gpu_data_C
        );
...
static_assert(Check_vector_kernel_1In_1Out_template<Vector_sum_atomic_kernel>::check_passed, "Vector_sum_atomic_kernel is not a valid kernel template");
