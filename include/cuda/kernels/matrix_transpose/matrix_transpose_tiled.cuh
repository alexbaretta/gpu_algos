...
__global__ void matrix_transpose_tiled(
    ...
    */
    ...
    #ifdef DEBUG
    // This declaration is useful to be able to display the contents of shm in the debugger.
    ...
    const long A_row = long(blockIdx.y) * long(blockDim.y) + long(threadIdx.y);
    const long A_col = long(blockIdx.x) * long(blockDim.x) + long(threadIdx.x);
    ...
    if (A_row < A_nrows && A_col < A_ncols) {
        ...
        if constexpr (debug) {
            ...
            if (A_idx >= n_elems) {
                printf("Bad indexing: A_idx=%ld at xy=(%ld,%ld) when A is %ldx%ld = %ld\n",
        ...
        printf("No can read at xy=(%ld,%ld)\n", A_row, A_col);
    }
