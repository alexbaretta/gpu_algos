...
// source path: include/cuda/kernel_api/tensor3d_3in_1out.cuh
...
concept TENSOR3D_KERNEL_SPEC_3IN_1OUT = requires (Kernel_spec spec) {
    ...
    { spec.size_temp_ } -> std::same_as<const long&>;
...
struct Check_tensor3d_kernel_spec_3In_1Out {
    ...
    static_assert(std::same_as<decltype(std::declval<Kernel_spec>().n_sheets_D_), const long>);

    static_assert(std::same_as<decltype(std::declval<Kernel_spec>().size_temp_), const long>);
    ...
    static_assert(TENSOR3D_KERNEL_SPEC_3IN_1OUT<Kernel_spec>, "not a valid TENSOR3D_KERNEL_SPEC_3IN_1OUT");

    constexpr static bool check_passed = true;
};

template <typename Kernel>
concept TENSOR3D_KERNEL_3IN_1OUT = requires (Kernel kernel) {
    ...
    { kernel.run_host_kernel(
        ...
        std::declval<const Tensor3D<typename detect::Numbers<Kernel>::C>&>()
...
struct Check_tensor3d_kernel_3In_1Out_template {
    static_assert(Check_tensor3d_kernel_3In_1Out<Kernel<std::int8_t>>::check_passed);
    static_assert(Check_tensor3d_kernel_3In_1Out<Kernel<std::uint8_t>>::check_passed);
    static_assert(Check_tensor3d_kernel_3In_1Out<Kernel<__half>>::check_passed);
    static_assert(Check_tensor3d_kernel_3In_1Out<Kernel<float>>::check_passed);
    static_assert(Check_tensor3d_kernel_3In_1Out<Kernel<double>>::check_passed);
