...
// source path: include/hip/kernels/matrix/matrix_transpose_tiled.hpp
...
__global__ void matrix_transpose_tiled(
    ...
    */
    /*
        HIP doesn't immediately 'support' dynamically-allocated shared memory arrays in templated functions, as it (apparently)
        ...
        Quoted from: See https://stackoverflow.com/questions/20497209/getting-hip-error-declaration-is-incompatible-with-previous-variable-name

        As a consequence we need to use a statically-allocated shared memory array with a maximum size of TILE_DIM * TILE_DIM.
    */
    __shared__ HIP_Number shm[TILE_DIM][TILE_DIM + 1];
    #ifdef DEBUG
    [[maybe_unused]] const HIP_Number* __shm = (HIP_Number*)shm;
    #endif
