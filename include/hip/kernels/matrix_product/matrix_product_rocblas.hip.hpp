...
// source path: include/hip/kernels/matrix/matrix_product_rocblas.hip.h
...
static_assert(Check_kernel_spec_2In_1Out<Matrix_product_rocblas_spec>::check_passed, "Matrix_product_rocblas_spec is not a valid kernel spec");
...
class Matrix_product_rocblas_kernel {
    ...
    void run_device_kernel(
        ...
        if constexpr (std::is_same_v<Number, rocblas_half>) {
            // Use rocBLAS for half precision
            rocblas_hgemm(rocblas_handle_,
                         rocblas_operation_none, rocblas_operation_none,
                         spec_.k_, spec_.m_, spec_.n_,
                         reinterpret_cast<const rocblas_half*>(&alpha),
                         reinterpret_cast<const rocblas_half*>(gpu_data_B), spec_.k_,
                         reinterpret_cast<const rocblas_half*>(gpu_data_A), spec_.n_,
                         reinterpret_cast<const rocblas_half*>(&beta),
                         reinterpret_cast<rocblas_half*>(gpu_data_C), spec_.k_);
        } else if constexpr (std::is_same_v<Number, float>) {
            // Use rocBLAS for single precision
            ...
            // Use rocBLAS for double precision
...
static_assert(Check_kernel_2In_1Out_template<Matrix_product_rocblas_kernel>::check_passed, "Matrix_product_rocblas is not a valid kernel template");
