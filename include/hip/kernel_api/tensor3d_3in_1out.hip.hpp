...
// source path: include/hip/kernel_api/tensor3d_3in_1out.hip.hpp
...
struct Check_tensor3d_kernel_spec_3In_1Out {
    ...
    static_assert(std::same_as<decltype(std::declval<Tensor3d_kernel_spec_3In_1Out>().block_dim_), const dim3>);
    static_assert(std::same_as<decltype(std::declval<Tensor3d_kernel_spec_3In_1Out>().grid_dim_), const dim3>);

    static_assert(std::same_as<decltype(std::declval<Tensor3d_kernel_spec_3In_1Out>().dynamic_shared_mem_words_), const size_t>);
