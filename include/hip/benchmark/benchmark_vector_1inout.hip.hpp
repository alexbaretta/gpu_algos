...
// source path: include/hip/benchmark/benchmark_vector_1inout.hip.hpp
...
class Benchmark_Vector_1Inout {
    ...
    int run() {
        ...
        hip_check_error(hipMemcpyAsync(vec_A_result_gpu.data(), gpu_data_A, size_A_bytes, hipMemcpyDeviceToHost, stream), "hipMemcpyAsync");
        if (size_temp_bytes > 0) {
