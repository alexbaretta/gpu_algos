...
#include "common/kernel_api/tensor3d_1in_1out.hpp"
...
class Benchmark_Tensor3D_1In_1Out {
    ...
    int run() {
        ...
        hip_check_error(cudaMemcpyAsync(gpu_data_A, A.data.data(), size_A_bytes, cudaMemcpyHostToDevice, stream), "cudaMemcpyAsync");
        ...
        hip_check_error(cudaMemcpyAsync(C.data.data(), gpu_data_C, size_C_bytes, cudaMemcpyDeviceToHost, stream), "cudaMemcpyAsync");
        if (size_temp_bytes > 0) {
            hip_check_error(cudaMemcpyAsync(temp.data.data(), gpu_data_temp, size_temp_bytes, cudaMemcpyDeviceToHost, stream), "cudaMemcpyAsync");
        }
