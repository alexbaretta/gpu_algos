...
class Benchmark_Tensor3D_3In_1Out {
    ...
    int run() {
        ...
        hip_check_error(cudaMemcpyAsync(gpu_data_A, A.data(), size_A_bytes, cudaMemcpyHostToDevice, stream), "cudaMemcpyAsync");
        hip_check_error(cudaMemcpyAsync(gpu_data_B, B.data(), size_B_bytes, cudaMemcpyHostToDevice, stream), "cudaMemcpyAsync");
        hip_check_error(cudaMemcpyAsync(gpu_data_C, C.data(), size_C_bytes, cudaMemcpyHostToDevice, stream), "cudaMemcpyAsync");
        ...
        hip_check_error(cudaMemcpyAsync(D.data(), gpu_data_D, size_D_bytes, cudaMemcpyDeviceToHost, stream), "cudaMemcpyAsync");
        if (size_temp_bytes > 0) {
            hip_check_error(cudaMemcpyAsync(temp.data(), gpu_data_temp, size_temp_bytes, cudaMemcpyDeviceToHost, stream), "cudaMemcpyAsync");
        }
        ...
        // Clean up
        hip_check_error(cudaEventDestroy(e0), "cudaEventDestroy");
        hip_check_error(cudaEventDestroy(e1), "cudaEventDestroy");
        hip_check_error(cudaEventDestroy(e2), "cudaEventDestroy");
        hip_check_error(cudaEventDestroy(e3), "cudaEventDestroy");
        hip_check_error(cudaEventDestroy(e4), "cudaEventDestroy");
        hip_check_error(cudaEventDestroy(e5), "cudaEventDestroy");
        hip_check_error(cudaStreamDestroy(stream), "cudaStreamDestroy");
