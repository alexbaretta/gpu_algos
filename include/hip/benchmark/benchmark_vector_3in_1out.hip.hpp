...
class Benchmark_Vector_3In_1Out {
    ...
    int run() {
        ...
        const Eigen::Map<Eigen::Vector<Number, Eigen::Dynamic>> B{vec_B.data(), spec.n_B_};
        const Eigen::Map<Eigen::Vector<Number, Eigen::Dynamic>> C{vec_C.data(), spec.n_C_};
        const Eigen::Map<Eigen::Vector<Number, Eigen::Dynamic>> D_gpu{vec_D.data(), spec.n_D_};
